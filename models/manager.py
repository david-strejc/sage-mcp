...
class ModelManager:
    ...
    def get_tool_description_hints(self) -> str:
        ...
        description_parts.append('\nUSAGE: Set model="gemini-2.5-pro" or model="o3" etc.')
    ...
    def select_model_for_task(
        ...
        if sorted_models:
            ...
            reasoning = self._generate_selection_reasoning(
                selected, mode, complexity, estimated_tokens, file_count
            )
    ...
    def _determine_complexity(self, mode: str, tokens: int, files: int) -> str:
        ...
        high = rules.get("high", {})
        if (files >= high.get("file_count", 5) or 
            tokens >= high.get("token_count", 50000) or
            mode in ["think", "analyze"]):
            return "high"
        
        # Check medium complexity
        medium = rules.get("medium", {})
        if (files >= medium.get("file_count", 2) or
            tokens >= medium.get("token_count", 10000) or
            mode in ["debug", "review", "plan", "refactor", "test"]):
            return "medium"
    ...
    def get_model_enum_with_descriptions(self) -> List[Dict[str, str]]:
        ...
        for model_name, config in self.models.items():
            ...
            result.append({
                "value": model_name,
                "description": f"{emoji} {model_name}: {hint} ({context_str})"
            })
