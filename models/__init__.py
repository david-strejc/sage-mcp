...
def select_model_for_task(mode: str, prompt_size: int, file_count: int = 0, 
                          conversation_context=None, allowed_models=None):
    """Select best model for a task"""
    return _manager.select_model_for_task(
        mode, prompt_size, file_count, conversation_context, allowed_models
    )

def get_model_hints():
    """Get model hints for display"""
    return {
        model: _manager.get_model_hint(model)
        for model in _manager.get_all_models()
    }
