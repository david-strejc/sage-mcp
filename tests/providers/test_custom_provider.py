...
class TestCustomProvider:
    ...
    async def test_basic_text_completion(self):
        ...
        messages = [
            {"role": "user", "content": "Say 'Hello from Custom provider test' and nothing else."}
        ]
    ...
    async def test_local_model_inference(self):
        ...
        messages = [
            {"role": "user", "content": "What is 6 * 7? Answer with just the number."}
        ]
    ...
    async def test_different_temperature_settings(self):
        ...
        messages = [
            {"role": "user", "content": "Write a creative name for a pet robot. Just one name."}
        ]
    ...
    async def test_error_handling(self):
        ...
        with pytest.raises(Exception):
            await self.provider.complete(
                messages=messages,
                model="nonexistent-local-model-12345",
                temperature=0.5
            )
