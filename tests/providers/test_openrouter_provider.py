...
class TestOpenRouterProvider:
    ...
    async def test_basic_text_completion(self):
        ...
        messages = [
            {"role": "user", "content": "Say 'Hello from OpenRouter provider test' and nothing else."}
        ]
    ...
    async def test_free_model_access(self):
        ...
        messages = [
            {"role": "user", "content": "Count from 1 to 3. Just the numbers separated by spaces."}
        ]
    ...
    async def test_premium_model_access(self):
        ...
        messages = [
            {"role": "user", "content": "What's the square root of 144? Just the number."}
        ]
    ...
    async def test_error_handling(self):
        ...
        with pytest.raises(Exception):
            await self.provider.complete(
                messages=messages,
                model="nonexistent-model-12345/test",
                temperature=0.5
            )
