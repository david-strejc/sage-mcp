...
class TestGeminiProvider:
    ...
    async def test_basic_text_completion(self):
        ...
        messages = [
            {"role": "user", "content": "Say 'Hello from Gemini provider test' and nothing else."}
        ]
    ...
    async def test_gemini_2_0_flash(self):
        ...
        messages = [
            {"role": "user", "content": "What's the capital of France? Answer in exactly two words."}
        ]
    ...
    async def test_thinking_mode_support(self):
        ...
        messages = [
            {"role": "user", "content": "Think step by step: If I have 3 apples and buy 2 more, then give away 1, how many do I have?"}
        ]
        ...
        try:
            ...
            response = await self.provider.complete(
                messages=messages,
                model="gemini-2.0-flash-thinking-exp",  # Thinking model
                temperature=0.3
            )
            assert response
    ...
    async def test_vision_capabilities(self):
        ...
        try:
            ...
            response = await self.provider.complete(
                messages=messages,
                model="gemini-1.5-pro",  # Pro model for better vision
                temperature=0.1
            )
            assert response
    ...
    async def test_large_context_window(self):
        ...
        messages = [
            {"role": "user", "content": f"Count how many times the word 'test' appears in this text: {large_text}. Just give me the number."}
        ]
    ...
    async def test_json_mode(self):
        ...
        messages = [
            {"role": "user", "content": "Return JSON with keys 'name' set to 'test' and 'count' set to 42. Return only valid JSON."}
        ]
    ...
    async def test_error_handling(self):
        ...
        with pytest.raises(Exception):
            await self.provider.complete(
                messages=messages,
                model="nonexistent-gemini-model-12345",
                temperature=0.5
            )
