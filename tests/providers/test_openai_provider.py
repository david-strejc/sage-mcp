...
class TestOpenAIProvider:
    ...
    async def test_basic_text_completion(self):
        ...
        messages = [
            {"role": "user", "content": "Say 'Hello from OpenAI provider test' and nothing else."}
        ]
    ...
    async def test_o1_reasoning_model(self):
        ...
        messages = [
            {"role": "user", "content": "Think step by step: What is 15 + 27?"}
        ]
        ...
        try:
            ...
            response = await self.provider.complete(
                messages=messages,
                model="o1-mini",
                temperature=1.0  # Should be handled properly for o1 models
            )
            assert response
