...
class TestRunner:
    ...
    def run_pytest(self, test_path, category, description):
        ...
        cmd = [sys.executable, "-m", "pytest", str(test_path), "-v" if self.verbose else "", "--tb=short"]
    ...
    def run_individual_test(self, test_script, category, description):
        ...
        try:
            result = subprocess.run(
                cmd, capture_output=True, text=True, timeout=120, cwd=self.project_root  # 2 minute timeout
            )
